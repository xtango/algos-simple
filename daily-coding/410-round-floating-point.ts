...
const roundUpDownArray = (inputFloatArr: number[]): number[] => {
    ...
    const helper = (
        ...
        if (i > inputFloatArr.length - 1) {
            console.log('\tHit last. Rounded sums equal?', cumulRoundedSum, inputFloatArrRoundedSum )
            return (cumulRoundedSum === inputFloatArrRoundedSum)
                ? {
                    diff: cumulDiff,
        ...
        // recurse
        const restUp = helper(i + 1, [currCeil, ...roundedArrSubset], cumulDiff + Math.abs(curr - currCeil));
        const restDown = helper(i + 1, [currFloor, ...roundedArrSubset], cumulDiff + Math.abs(curr - currFloor));
...
// console.log(absoluteDiff([1.3, 2.3, 4.4], [1, 2, 5]))
 console.log(roundUpDownArray([1.3, 2.3, 4.4]))
